...
name: todos
description: >-
  Sync todos with todos.in (webhook from `.env`, `$TODOS_WEBHOOK`, or `todos -w
  URL`): run `todos`, edit
...
1. Ensure a webhook is configured: `TODOS_WEBHOOK` in `.env`, exported `TODOS_WEBHOOK`, or `todos -w <url>` for that run.
